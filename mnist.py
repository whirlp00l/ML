...
class MNIST(object):
    ...
    def load(cls, path_img, path_lbl):
        ...
        for i in xrange(size):
            images.append([0]*(rows*cols+1)) #add initial 1 for learning

        for i in xrange(size):
            images[i][0] = 1;
            images[i][1:1+rows*cols] = image_data[i*rows*cols : (i+1)*rows*cols]
    ...
    def test(self):
        ...
        print 'Showing num:%d' % train_label[59999]
        print self.display(train_img[59999])
        print
        return True
